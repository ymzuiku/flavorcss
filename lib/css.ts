...
function mkCss(n: string, v: string) {
  ...
  [["first", "first-child"], ["last", "last-child"], ["odd", "nth-child(odd)"]].forEach(([h, h2]) => {
    css += `.${h}\\:${n}:${h2}{${v}} `;
...
export const sheet = {} as any;
function mk(n: string, v: string) {
  sheet[n] = v;
}
...
export function mkStyle(str: string, id: string) {
