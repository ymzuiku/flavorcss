...
const details = [
  {
    ...
    info: "Flavor CSS only import a js library, Using CSS variable reasonably, modifying the value of atomic class does not need rebuild.",
  },
  {
    title: "Infinite atomic classes",
    info: "Flavor supports all inline styles by default. Conventional Atomic class with different values, media queries, pseudo class combination, there will be a very large number. If we only use part of them, we will need to leave HTML. Flavor CSS dynamically compiles and adds the atomic classes we use.",
  },
  {
    title: "Rich functions",
    info: "Support more than 20 kinds of media query, including screen width, device type, dark; support nearly complete pseudo class; support CSS selector.",
  },
  {
    title: "Dynamic build css",
    info: "Dynamic build css when insert new html element in body. You can use flavorcss in codeless or PAAS system.",
  },
];
...
export function App() {
  return (
    <div class="max-width:100vw overflow:hidden md:max-width:1100px margin:0|auto">
      <Header />
      <div class="margin-top:40px sm:margin-top:100px padding:--lg lg:padding:--xl grid-gap:--xxl display:grid place-content:center">
        <div class="max-w:900px">
          <h1 class="lg:text-align:center font-size:46px lg:font-size:56px xl:font-size:60px max-width:900px font-weight:800 line-height:1">
            Don't need to memorize key words' <span class="color:hsl(150,50%,50%)">Runtime</span> atomic class CSS
            library
          </h1>
        ...
        <div class="margin:0|auto display:grid grid-template-rows:1fr|1fr sm:grid-template-rows:1fr sm:grid-template-columns:1fr|1fr margin:--md|auto margin-top:40px max-width:800px grid-gap:--lg">
          <Card h={200} title="Read the docs" info="Learn how to use flavor." button="Start learning" url="/doc" />
          <Card
            h={155}
            title="The examples"
      ...
      <div class="margin:100px|20px">
        <section class="xxl:text-align:center:* max-width:1100px margin:0|auto display:grid md:place-content:center max-width:1000px">
          ...
          <h4 class="color:hsl(200,40%,30%) font-size:22px font-weight:300 margin:20px|auto">
            The scheme of flavourcss avoids the problem of how to determine the boundary of the number of atomic
            classes. Flavourcss compiles atomic class at runtime, which directly solves the volume and engineering
            ...
            return (
              <setcion class="max-width:400px display:grid grid-template-columns:50px|auto grid-gap:20px">
                ...
                <div class="diplay:grid grid-template-rows:50px|auto">
                  <h3 class="font-weight:500 font-size:20px color:hsl(220,50%,20%)">{item.title}</h3>
                  <h4 class="font-weight:300 font-size:16px color:hsl(200,50%,35%)">{item.info}</h4>
                </div>
              </setcion>
            );
          })}
      ...
      <div class="margin:20px|auto max-width:1000px padding:20px">
        <h1 class="margin-top:40px font-size:40px font-weight:800 line-height:1">Getting started</h1>
        <p class="md-info">
          Flavor don't neet hundreds and thousands of css class list, replace by a grammatical rule.
        </p>
        <h1 class="margin-top:20px font-size:30px font-weight:800 line-height:1">Case 1:</h1>
        <p class="md-info">Use unpkg modules:</p>
        <p class="md-code">
          &lt;script type=&quot;module&quot;
          src=&quot;https://unpkg.com/flavorcss@0.7.0/esm/index.js&quot;&gt;&lt;/script&gt;
        </p>
        <h1 class="margin-top:20px font-size:30px font-weight:800 line-height:1">Case 2:</h1>
